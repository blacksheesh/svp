...
void SignalGenerator::generateSignal() {
    ...
    QPen yAxisPen(QColor(57, 130, 57));  // Ось Y
    yAxisPen.setWidth(4);
    QGraphicsLineItem *yAxis = new QGraphicsLineItem(0, -graphicsView->height() * 0.5, 0, graphicsView->height() * 0.5);
    ...
    for (int i = 0; i < numPoints - 1; ++i) { //numPoints представляет собой количество точек на графике
        ...
        double phaseShift = phaseSpinBox->value();
        double value1 = amplitudeSpinBox->value() * qSin(2 * M_PI * frequencySpinBox->value() * t1 + qDegreesToRadians(phaseShift));
        double value2 = amplitudeSpinBox->value() * qSin(2 * M_PI * frequencySpinBox->value() * t2 + qDegreesToRadians(phaseShift)); //начения value1 и value2 представляют значения сигнала в моменты времени t1 и t2 вычисляются, используя синусоидальную функцию с учетом частоты, амплитуды и сдвига фазы.
